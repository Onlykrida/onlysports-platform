...
**Sports talent discovery for India.** We make grassroots athletes findable by scouts, using measured performance instead of personal networks.

Most Indian athletes never get seen — not because they lack talent, but because scouting runs on who you know and proximity to a handful of cities. OnlyKrida gives every athlete standardised, comparable results and a profile a scout can filter for.

**Claim discipline:** do not write "India's first", "99% never get scouted", "50L+ athletes", or engagement multipliers ("2x/3x/5x more engagement") into product copy, marketing, or docs. None are sourceable and the multipliers have no computation anywhere in the codebase. State mechanisms, not invented numbers.
...
- **AI (Codex)** powers smart scouting recommendations, profile coaching, opportunity matching, and an in-app assistant (Krida AI).
...
**React Native (Expo SDK 54) + TypeScript + Supabase + Codex AI**
...
**Anthropic API key** is NOT in `.env` anymore. Codex calls go through the
`Codex-proxy` Supabase Edge Function (see `supabase/functions/Codex-proxy/`),
...
- `ai-context.tsx` — Codex AI features (chat, profile coach, scout recommendations)
...
- **Model selection** is via `options.useSmartModel`:
  - `Codex-sonnet-4-6` (default) for chat, summaries, profile tips. ~300–800 tokens.
  - `Codex-opus-4-6` for scout matching and opportunity matching. ~1000–1500 tokens, with `options.useThinking = true` (sets `thinking: { type: 'adaptive' }` in the body).
...
This section covers Codex sessions that use gstack skills and gbrain knowledge brain.
...
gstack lives at `~/.Codex/skills/gstack`. Skills load automatically per session.
...
- `/document-release` — Updates AGENTS.md and docs/ after shipping.
...
Create `.Codex/server.json` in the repo root (not `~/.Codex/`):
...
This keeps gbrain active only for OnlyKrida sessions, not every Codex session.
