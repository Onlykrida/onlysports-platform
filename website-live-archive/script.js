...
(function () {
  ...
  // Cursor hover states
  const hoverTargets = document.querySelectorAll('a, button, [data-magnetic], .sticker, .feature-card, .polaroid, .role');
  ...
  const revealTargets = document.querySelectorAll(
    '.problem__inner, .number-block, .cta__inner'
  );
  ...
  // ============================================
  const zoomEls = document.querySelectorAll('.problem, .features, .roles, .culture, .app-preview, .velocity, .beep-splash');
  ...
  if (ctaLine1) {
    const ctaSplitObs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
  ...
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRjaXhsZXJuZXV1eWhzZnRuaWZtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU3MDc4NTIsImV4cCI6MjA3MTI4Mzg1Mn0.VN7zdfRWHIhTSWQ0HMEhuBKZ49J7Ks4PHtOB140Yn-c';
  ...
  if (ctaForm) {
    ctaForm.addEventListener('submit', async (e) => {
      ...
      // The label lives in a child <span> (data-magnetic hover effect); writing to
      // textContent directly would destroy that span and break the animation.
      ...
      try {
        ...
        // 409 = unique violation on lower(email): this address is already on the
        // list. That is a success from the visitor's point of view, not an error.
        if (res.status === 409) {
  ...
  function onScroll() {
    ...
    if (sportLayers.length >= 3) {
      ...
      sportLayers.forEach(layer => { layer.style.opacity = breathe; });
    }
  ...
  if (phoneSection) {
    const phoneSlideObs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
  ...
  if (velocitySection) {
    const velObs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          entry.target.classList.add('section-visible');
        } else {
  ...
  if (beepCounter) {
    ...
    const beepObs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !beepRunning) {
          ...
          const tick = () => {
            ...
            setTimeout(() => { beepCounter.style.transform = 'scale(1)'; }, 150);
            level++;
            if (level <= maxLevel) {
  ...
  document.querySelectorAll('.sticker').forEach(sticker => {
    sticker.addEventListener('mouseenter', () => {
      sticker.style.transition = 'transform 0.15s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.3s';
    });
    sticker.addEventListener('mouseleave', () => {
      sticker.style.transition = 'transform 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94), box-shadow 0.3s';
    });
  });
