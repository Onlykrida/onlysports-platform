-- Complete OnlySports Database Setup and Fix Script
...
-- Sample profiles (these will only be inserted if they don't exist)
INSERT INTO public.profiles (id, email, name, role, sport, bio, location, verified)
SELECT 
    gen_random_uuid(),
    'sample.athlete@example.com',
    'Sample Athlete',
    'athlete',
    'Football',
    ...
    'New York, USA',
    true
WHERE NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'sample.athlete@example.com');

INSERT INTO public.profiles (id, email, name, role, sport, bio, location, verified)
SELECT 
    gen_random_uuid(),
    'sample.coach@example.com',
    'Sample Coach',
    'coach',
    'Basketball',
    ...
    'Los Angeles, USA',
    true
WHERE NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'sample.coach@example.com');

INSERT INTO public.profiles (id, email, name, role, sport, bio, location, verified)
SELECT 
    gen_random_uuid(),
    'sample.scout@example.com',
    'Sample Scout',
    'scout',
    'Soccer',
    ...
    'London, UK',
    false
WHERE NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'sample.scout@example.com');
