...
export const [GroupsProvider, useGroups] = createContextHook<GroupsState>(() => {
  ...
  const loadGroups = useCallback(async () => {
    ...
    try {
      ...
      // Get group details with member count and last message in a single query
      const { data: groupsData, error: groupsError } = await supabase
        .from('groups')
        .select(
          `
          id, name, avatar, created_by, created_at, updated_at,
          group_members(id),
          group_messages(content, created_at, sender_id, sender:profiles!group_messages_sender_id_fkey(name))
        `,
        )
        .in('id', groupIds);
      ...
      const groupList: Group[] = (groupsData || []).map((g: any) => {
        // group_messages comes back unordered — pick the most recent one
        const messages: any[] = g.group_messages || [];
        const lastMsg =
          messages.length > 0
            ? messages.reduce((a: any, b: any) => (a.created_at > b.created_at ? a : b))
            : null;
  ...
  const createGroup = useCallback(
    async (name: string, avatar: string | null, memberIds: string[]) => {
      if (!user || !isSupabaseConfigured) {
        return { error: 'Not authenticated or database not configured' };
      }
  ...
  // Keep a ref to groups so the real-time callback always has the latest list
  // without needing `groups` in the dependency array (which would cause
  // unsubscribe/resubscribe on every group list change).
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          // Check if user is in this group (from latest state via ref)
          const isInGroup = groupsRef.current.some((g) => g.id === msg.group_id);
          if (!isInGroup) return;
