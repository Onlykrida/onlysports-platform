...
import { Mail, Lock, User as UserIcon, Search, Building2, MapPin } from 'lucide-react-native';
...
export default function SignupScoutScreen() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const upd = await updateProfile({ bio: `Org: ${organization}. Regions: ${regions}. Sports: ${sports}`, location: location || undefined });
      if (upd.error) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.form}>
            ...
            <Input label="Organization" placeholder="Team/Agency name" value={organization} onChangeText={setOrganization} icon={<Building2 size={20} color={theme.colors.textSecondary} />} />
            <Input label="Regions of Interest" placeholder="e.g., South Asia, GCC" value={regions} onChangeText={setRegions} icon={<Search size={20} color={theme.colors.textSecondary} />} />
            <Input label="Sports Focus" placeholder="e.g., Football, Cricket" value={sports} onChangeText={setSports} icon={<Search size={20} color={theme.colors.textSecondary} />} />
            <Input label="City, Country" placeholder="e.g., Dubai, AE" value={location} onChangeText={setLocation} icon={<MapPin size={20} color={theme.colors.textSecondary} />} />
          </View>
